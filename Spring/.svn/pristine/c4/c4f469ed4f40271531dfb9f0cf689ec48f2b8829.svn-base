...
public class MyprofileService {
	...
	public void getUserInfo(Model model, int user_id){
		...
		int AGE = Year.now().getValue() - Integer.parseInt(user_info.getBIRTHDAY().split("-")[0]) + 1;
		user_info.setAGE(String.valueOf(AGE));
	...
	public void getUserClub(Model model, int user_id){
		...
		user_club = dao.getUserFed(dto);
		model.addAttribute("user_fed", user_club);
	}
...
//		}
	...
	public Object getoutFromClub(ClubUserDto dto, int user_id) {
		AccountDao dao = sqlSession.getMapper(AccountDao.class);
		dto.setUSER_ID(user_id);
		dao.getoutFromClub(dto);
		JsonDto Jdto = new JsonDto();
		Jdto.setRESULT(1);
		return Jdto;
	}
